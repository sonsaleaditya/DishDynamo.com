...
const getAllRecipes = async (req, res) => {
  try {
    ...
    res.status(200).json({ success : true,  allRecipes});
  } catch (error) {
    console.error(error);
    res.status(500).json({ success :false, error: "Internal server error" });
  }
...
const deleteRecipe = async (req, res) => {
  try {
    ...
    res.status(200).json({ success : true, message: "Recipe deleted successfully", recipes });
  } catch (error) {
    console.error(error);
    res.status(500).json({success : false, error: "Internal server error" });
  }
...
const LikedList = async (req, res) => {
  try {
    ...
    if (existingFavorite) {
      ...
      return res
        .status(400)
        .json({ existFav : true, error: "Recipe already exists in your favorites" });
    } else {
      ...
      return res.status(201).json({ success : true, favoriteRecipe: newFavorite });
    }
  } catch (error) {
...
const getAllLikedRecipes = async (req, res) => {
  try {
    ...
    res.status(200).json({ success : true , allLikedRecipes});
  } catch (error) {
    console.error(error);
    res.status(500).json({ success : false,  error: "Internal server error" });
  }
